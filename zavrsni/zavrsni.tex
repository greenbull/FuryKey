...
Jedinku u genetskom algoritmu predstavljala je jedna tipkovnica sa svojim oblikom i rasporedom tipki. Na početku se stvori populacija tipkovnica sa slučajno odabranim rasporedom tipki i slučajnim oblikom. Nakon toga se za svaku tipkovnicu izračuna vremenska cijena prema izrazu \ref{eq:fitts4} te se jedinke u populaciji uzlazno sortiraju, tako da je najbolja jedinka (ona s najmanjom vremenskom cijenom) na početku. Stvori se nova populacija koja je na početku prazan skup. Ta se populacija puni novim jedinkama (tipkovnicama) križanjem, mutiranjem i reprodukcijom sve dok broj jedinki ne bude isti kao i u trenutnoj populaciji. Nova populacija tada postaje trenutna populacija koja se opet evaluira i tako u krug sve dok se ne dosegne maksimalan broj generacija (zadan proizvoljno). Najbolja tipkovnica u posljednjoj generaciji predstavlja rješenje algoritma.
...
U poglavlju 2.1 opisana je problematika i dobrim dijelom pristup rješavanju problema. U ovom poglavlju ćemo se detaljnije pozabaviti načinu na koji sam implementirao algoritam i rezultatima koje sam dobio.
\\pitat glavinića u vezi opisa koda i te pizdarije 
